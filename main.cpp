...
void ProcessProblem(const std::string &dense_folder, const std::vector<Problem> &problems, const int idx, bool geom_consistency, bool planar_prior, bool hierarchy, bool multi_geometrty=false)
{
    ...
    GST gst;
    if (geom_consistency) {
        gst.SetGeomConsistencyParams(multi_geometrty);
    }
    if (hierarchy) {
        gst.SetHierarchyParams();
    }

    gst.InuputInitialization(dense_folder, problems, idx);

    gst.CudaSpaceInitialization(dense_folder, problem);
#if COSTUPDATE
    gst.SetDataPasserInCuda();
#endif
    gst.RunPatchMatch();

    const int width = gst.GetReferenceImageWidth();
    const int height = gst.GetReferenceImageHeight();
    ...
    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            ...
            float4 plane_hypothesis = gst.GetPlaneHypothesis(center);
            depths(row, col) = plane_hypothesis.w;
            normals(row, col) = cv::Vec3f(plane_hypothesis.x, plane_hypothesis.y, plane_hypothesis.z);
            costs(row, col) = gst.GetCost(center);
        }
    }

    if (planar_prior) {
        std::cout << "Run Planar Prior Assisted PatchMatch MVS ..." << std::endl;
        gst.SetPlanarPriorParams();

        const cv::Rect imageRC(0, 0, width, height);
        std::vector<cv::Point> support2DPoints;

        gst.GetSupportPoints(support2DPoints);
        const auto triangles = gst.DelaunayTriangulation(imageRC, support2DPoints);
        cv::Mat refImage = gst.GetReferenceImage().clone();
        ...
        for (const auto triangle : triangles) {
            if (imageRC.contains(triangle.pt1) && imageRC.contains(triangle.pt2) && imageRC.contains(triangle.pt3)) {
                ...
                float4 n4 = gst.GetPriorPlaneParams(triangle, depths);
        ...
        for (int i = 0; i < width; ++i) {
            for (int j = 0; j < height; ++j) {
                if (mask_tri(j, i) > 0) {
                    float d = gst.GetDepthFromPlaneParam(planeParams_tri[mask_tri(j, i) - 1], i, j);
                    if (d <= gst.GetMaxDepth() && d >= gst.GetMinDepth()) {
                        priordepths(j, i) = d;
                    }
                    else {
                        mask_tri(j, i) = 0;
        ...
        gst.CudaPlanarPriorInitialization(planeParams_tri, mask_tri);
        gst.RunPatchMatch();
...
void JointBilateralUpsampling(const std::string &dense_folder, const Problem &problem, int gst_size)
{
    std::stringstream result_path;
    result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    ...
    const float factor_x = static_cast<float>(gst_size) / image_float.cols;
    const float factor_y = static_cast<float>(gst_size) / image_float.rows;
...
void JointBilateralUpsampling_new(const std::string &dense_folder, const Problem &problem, int gst_size)
{
    std::stringstream result_path;
    result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    ...
    std::cout << "Run JBU_new for image " << problem.ref_image_id <<  ".jpg" << std::endl;
    RunJBU_new(image_float, priordepths_scale0, priordepth_upsample_scale0, priornormals_scale0, priornormal_upsample_scale0,dense_folder, problem );
...
void RunFusion(std::string &dense_folder, const std::vector<Problem> &problems, bool geom_consistency)
{
    ...
    for (size_t i = 0; i < num_images; ++i) {
        ...
        std::stringstream result_path;
        result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problems[i].ref_image_id;
        std::string result_folder = result_path.str();
        std::string suffix = "/depths.dmb";
        if (geom_consistency) {
            suffix = "/depths_geom.dmb";
    ...
    std::string ply_path = dense_folder + "/GST/GST_model.ply";
...
{
    if (argc < 2) {
        std::cout << "USAGE: GST dense_folder" << std::endl;
        return -1;
    }
    ...
    std::string output_folder = dense_folder + std::string("/GST");
    mkdir(output_folder.c_str(), 0777);

    size_t num_images = problems.size();
    std::cout << "There are " << num_images << " images needed to be processed!" << std::endl;
