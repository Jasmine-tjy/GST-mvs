...
#if TEXTURE_BASED_SAMPLING
void readTextureCsv(const std::string& filename, std::vector<cv::Mat>& texture) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error opening csv file: " << filename << std::endl;
        if (file.fail()) {
            std::cerr << "failed: " << file.rdstate() << std::endl;
        }
        return;
    }
    std::string line;
    ...
    while (std::getline(file, line)) {
        ...
        row.clear(); // Clear row vector to prepare for the next row
        while (std::getline(lineStream, cell, ',')) {
            float value;
            if (std::stringstream(cell) >> value) {
                row.push_back(value);
            } else {
                std::cerr << "Error converting string to float: " << cell << std::endl;
                return;
            }
        }
        // After reading an entire line, create a cv::Mat from the row and add it to the vector
        cv::Mat rowMat(row.size(), 1, CV_32F, row.data());
        texture.push_back(rowMat);
    }
}

std::vector<cv::Mat> readTextureCsv1(const std::string& filename) {
    std::vector<cv::Mat> textureValue;
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error opening csv file: " << filename << std::endl;
        if (file.fail()) {
        ...
        return textureValue; // Return empty vector if file opening failed
    }
    std::string line;
    while (std::getline(file, line)) {
        std::istringstream lineStream(line);
        ...
        // Create a cv::Mat from the row and add it to the vector
        cv::Mat rowMat(1, row.size(), CV_32F);
        for (size_t i = 0; i < row.size(); ++i) {
            rowMat.at<float>(0, i) = row[i];
        }
        textureValue.push_back(rowMat);
    }
    return textureValue;
}

cv::Mat readTextureCsv2(const std::string& filename, int rows, int cols) {
    cv::Mat textureValue(rows, cols, CV_32F); // Initialize Mat with the size of the image
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error opening csv file: " << filename << std::endl;
        if (file.fail()) {
        ...
        return textureValue; // Return empty Mat if file opening failed
    }
    int row = 0;
    std::string line;
    while (std::getline(file, line) && row < rows) {
        ...
        while (std::getline(lineStream, cell, ',')) {
            float value;
            if (std::stringstream(cell) >> value) {
                textureValue.at<float>(row, col) = value;
            } else {
                std::cerr << "Error converting string to float: " << cell << std::endl;
            }
            col++;
    ...
    if (row != rows) {
        std::cerr << "Warning: CSV file has fewer rows than expected." << std::endl;
    }
    return textureValue;
}

void readTextureCsv3(const std::string& filename, float* textureValue, int rows, int cols) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error opening csv file: " << filename << std::endl;
        if (file.fail()) {
        ...
        return; // File opening failed, nothing to read
    }
    int index = 0;
    std::string line;
    ...
    if (index != rows * cols) {
        std::cerr << "Warning: CSV file has fewer elements than expected." << std::endl;
    }
}
#endif
...
void GST::SetGeomConsistencyParams(bool multi_geometry=false)
{
...
void GST::SetHierarchyParams()
{
    params.hierarchy = true;
}

void GST::SetPlanarPriorParams()
{
    params.planar_prior = true;
}

void GST::InuputInitialization(const std::string &dense_folder, const std::vector<Problem> &problems, const int idx)
{
    images.clear();
    cameras.clear();
#if TEXTURE_BASED_SAMPLING
    //texture_sampling_value.clear();
    //texture_sampling_value.setTo(0);
    //delete[] texture_sampling_value;
    texture_sobelX.clear();
    texture_sobelY.clear();
#endif
...
#if TEXTURE_BASED_SAMPLING
        if (i == 0) { //only calculate the first image(reference)
        std::vector<cv::Mat> texture_sobelX(images.size());
        std::vector<cv::Mat> texture_sobelY(images.size());
        std::vector<cv::Mat> images_texture(images.size());
        //calculate for texture
        cv::Sobel(images[i], texture_sobelX[i], CV_32F, 1, 0);
        cv::Sobel(images[i], texture_sobelY[i], CV_32F, 0, 1);
        //path
        cv::magnitude(texture_sobelX[i], texture_sobelY[i], images_texture[i]);
        std::stringstream result_file_path;
        result_file_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
        std::string result_folder = result_file_path.str();
        //image 
        /*cv::Mat convertedImage;
        images_texture[i].convertTo(convertedImage, CV_8UC1, 1.0, 0.0);
        if (convertedImage.empty()) {
            std::cerr << "convertedImage is empty." << std::endl;
            continue;
        }
        std::string texture_IMG_suffix = "/texture_" + std::to_string(i) + "_" + std::to_string(factor) + ".jpg";
        std::string texture_IMG_path = result_folder + texture_IMG_suffix;
        cv::imwrite(texture_IMG_path, convertedImage);*/
        //mat
        std::string texture_MAT_suffix = "/texture_" + std::to_string(i) + "_" + std::to_string(new_rows)+"x" + std::to_string(new_cols) + ".csv";
        std::string texture_MAT_path = result_folder + texture_MAT_suffix;
        if (images_texture[i].channels() != 1) {
            std::cerr << "Only single channel matrices can be saved to CSV." << std::endl;
            continue;
        }
        if (access(texture_MAT_path.c_str(), F_OK) != -1) {
            std::cout << "File already exists. Skipping write operation." << std::endl;
            continue;
        }
        else {
            std::ofstream texture_MAT_file(texture_MAT_path);
            if (!texture_MAT_file.is_open()) {
                std::cerr << "Could not open file for writing." << std::endl;
                continue;
            }
            //COSTUPDATE
            weak_count = 0;
            for (int t_row = 0; t_row < new_rows; ++t_row) {
                for (int t_col = 0; t_col < new_cols; ++t_col) {
                    if(images_texture[i].at<float>(t_row, t_col)<40){
                       weak_count++;
                    }
                    texture_MAT_file << images_texture[i].at<float>(t_row, t_col) << ",";
                }
                texture_MAT_file << std::endl;
            }
            texture_MAT_file.close();
        }
        //std::cout<<"written "<<texture_MAT_path<<" file DONE"<<std::endl;
            std::cout << "Weak count: " << weak_count << " / " << new_cols * new_rows << " = " << (float)weak_count / (float)(new_cols * new_rows) * 100 << "%" <<"   in figure"<<idx <<std::endl;
        }
    ...
    if (params.geom_consistency) {
        ...
        result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
        ...
        for (size_t i = 0; i < num_src_images; ++i) {
            std::stringstream result_path;
            result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.src_image_ids[i];
            std::string result_folder = result_path.str();
            std::string depth_path = result_folder + suffix;
            cv::Mat_<float> depth;
...
void GST::CudaSpaceInitialization(const std::string &dense_folder, const Problem &problem)
{
    ...
    for (int i = 0; i < num_images; ++i) {
        ...
        if (i == 0) {
            ...
            result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
            std::string result_folder = result_path.str();
            std::string texture_MAT_suffix = "/texture_" + std::to_string(i) + "_" + std::to_string(rows) + "x" + std::to_string(cols) + ".csv";
            //std::string texture_MAT_suffix = "/texture_0_" + std::to_string(rows) + "x" + std::to_string(cols) + ".csv";
            std::string texture_MAT_path = result_folder + texture_MAT_suffix;
            //readTextureCsv(texture_MAT_path, texture_sampling_value);
            //texture_sampling_value = readTextureCsv1(texture_MAT_path);
            float* texture_sampling_value = new float[cameras[0].height * cameras[0].width];
            readTextureCsv3(texture_MAT_path,texture_sampling_value,cameras[0].height,cameras[0].width);
            // float* dev_texture_data;
            cudaMalloc((void**)&texture_obj_sampling_cuda, sizeof(float)*(cameras[0].height * cameras[0].width));
            cudaMemcpy(texture_obj_sampling_cuda, texture_sampling_value, sizeof(float)*(cameras[0].height * cameras[0].width), cudaMemcpyHostToDevice);
            delete[] texture_sampling_value;
        }
 #endif
    }
    ...
    cudaMalloc((void**)(&neigbours_map_cuda), (cameras[0].height * cameras[0].width) * sizeof(int));
    cudaMemcpy(neigbours_map_cuda, neighbours_map_host.ptr<int>(0), (cameras[0].height * cameras[0].width) * sizeof(int), cudaMemcpyHostToDevice);
    // malloc memory for deformable ncc
    cudaMalloc((void **)(&neighbours_cuda), weak_count * 9 * sizeof(short2));
#endif
}

void GST::CudaPlanarPriorInitialization(const std::vector<float4> &PlaneParams, const cv::Mat_<float> &masks)
{
...
int GST::GetReferenceImageWidth()
{
    return cameras[0].width;
}

int GST::GetReferenceImageHeight()
{
    return cameras[0].height;
}

cv::Mat GST::GetReferenceImage()
{
    return images[0];
}

float4 GST::GetPlaneHypothesis(const int index)
{
    return plane_hypotheses_host[index];
}

float GST::GetCost(const int index)
{
    return costs_host[index];
}

float GST::GetMinDepth()
{
    return params.depth_min;
}

float GST::GetMaxDepth()
{
    return params.depth_max;
}

void GST::GetSupportPoints(std::vector<cv::Point>& support2DPoints)
{
...
std::vector<Triangle> GST::DelaunayTriangulation(const cv::Rect boundRC, const std::vector<cv::Point>& points)
{
    if (points.empty()) {
...
float4 GST::GetPriorPlaneParams(const Triangle triangle, const cv::Mat_<float> depths)
{
...
float GST::GetDepthFromPlaneParam(const float4 plane_hypothesis, const int x, const int y)
{
...
#if DEPTHMAP_SUPPLIMENT
void RunJBU_new(const cv::Mat_<float>  &scaled_image_float, const cv::Mat_<float> &src_depthmap,cv::Mat_<float>& upsample_depthmap, const cv::Mat_<cv::Vec3f>& src_normal, cv::Mat_<cv::Vec3f>& upsample_normal, const std::string &dense_folder , const Problem &problem)
{
    uint32_t rows = scaled_image_float.rows;
    uint32_t cols = scaled_image_float.cols;
    int Imagescale = std::max(scaled_image_float.rows / src_depthmap.rows, scaled_image_float.cols / src_depthmap.cols);
    ...
    // 为每个像素点分配一个float4类型的数组，用于存储法线和深度信息
    ...
    jbu.InitializeParameters_new(rows * cols, src_depthmap.rows * src_depthmap.cols);
    jbu.CudaRun();
    ...
    cv::Mat_<float> disp0 = upsample_depthmap.clone();
    std::stringstream result_path;
    result_path << dense_folder << "/GST" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    mkdir(result_folder.c_str(), 0777);
...
void JBU::InitializeParameters_new(int n, int origin_n)
{
    depth_h = (float*)malloc(sizeof(float) * n);
...
void GST::SetDataPasserInCuda() {
    ...
    cudaMalloc((void**)(&helper_cuda), sizeof(DataPasser));
    cudaMemcpy(helper_cuda, &helper_host, sizeof(DataPasser), cudaMemcpyHostToDevice);
}
#endif