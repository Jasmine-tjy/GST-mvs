...
#if TEXTURE_BASED_SAMPLING
    std::vector<cv::Mat> texture_sobelX;
    std::vector<cv::Mat> texture_sobelY;
    std::vector<cv::Mat> magnitudeDisplay; //for texture visualization
    //cudaTextureObject_t *texture_sampling_cuda;
    float *texture_obj_sampling_cuda;
    TexSampObj texture_obj_sampling_host;
    //cudaArray *cuTexArray[1];
    //cv::Mat texture_sampling_value;
    float* texture_sampling_value;
#endif
#if COSTUPDATE
    DataPasser helper_host;
    DataPasser *helper_cuda;
    int weak_count;
...
public:
    ...
    //functions
    void InitializeParameters_new(int n, int origin_n);
