...
#if TEXTURE_BASED_SAMPLING
__device__ float GetTextureValue(float* texSampObj, int2 p, int width, int height) {
    if (p.x >= 0 && p.x < width && p.y >= 0 && p.y < height) {
        // Calculate the linear index for the 1D array
        int index = p.y * width + p.x;
        // Access the value at the calculated index
        return texSampObj[index];
    } else {
        printf("wrong texture value!");
...
#if COSTUPDATE
__device__ short2 GetNeighbourPoint(const int2 p, int index, const DataPasser *passer) {
    const unsigned offset = passer->neighbours_map_cuda[p.x + p.y * passer->width] * 9;
    short2 neighbour_pt = passer->neighbours_cuda[offset + index];
    return neighbour_pt;
}

__device__ float ComputeBilateralNCCNew(const cudaTextureObject_t ref_image, const Camera ref_camera, const cudaTextureObject_t src_image, const Camera src_camera, const int2 p, const float4 plane_hypothesis, const PatchMatchParams params, int index, float textureDegree,const DataPasser *passer)
{
    const float cost_max = 2.0f;
    float H[9];
    ComputeHomography(ref_camera, src_camera, plane_hypothesis, H);
    ...
    if(textureDegree < 50)
    {
        ...
        int strong_count = 0;
        for (int k = 0; k < 9; ++k){
            const short2 neighbour_pt = GetNeighbourPoint(p, k, passer);
            ...
            if (neighbour_src_pt.x < 0 || neighbour_src_pt.y < 0 || neighbour_src_pt.x >= ref_camera.width || neighbour_src_pt.y >= ref_camera.height) {
                if (k != 0) {
                    unsigned int view_info = passer->selected_views_cuda[neighbour_pt.x + neighbour_pt.y * ref_camera.width];
                    if (isSet(view_info, index - 1)) {
...
#if TEXTURE_BASED_SAMPLING
__global__ void RandomInitialization(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses,  float4 *scaled_plane_hypotheses, float *costs,  float *pre_costs,  curandState *rand_states, unsigned int *selected_views, float4 *prior_planes, unsigned int *plane_masks, const PatchMatchParams params, float* texture_sampling)
{
    ...
    if (!params.geom_consistency && !params.hierarchy ) {
        ...
        //printf("planar_prior: true \n");
        //TEXTURE_BASED_SAMPLING
        float texture_degree;
        texture_degree = GetTextureValue(texture_sampling, p,width,height);
        //printf("w*h = %d * %d texture_degree at [%d,%d]: %f \n",width,height,p.x,p.y,texture_degree);
        //
        if (plane_masks[center] > 0 && costs[center] >= 0.1f) {
            float perturbation = 0.03f;
...
#if TEXTURE_PROPAGATION
__device__ bool BoarderDeter(const int iter, int Extended_iter, int orientation, float(*cost_array)[32], const PatchMatchParams params, int flag,float textureDegree) {
    if (!flag) {
        return false;
    }
    ...
    if (good_sum >= 1 && bad_sum <= 2 && textureDegree>90) {
        return false;
    }
    else {
        return true;
...
__device__ void BoarderUpFarPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderDownFarPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderLeftFarPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderRightFarPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderLeftUpPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderRightUpPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderRightDownPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void BoarderLeftDownPropagation(const cudaTextureObject_t* images, const Camera* cameras, float4* plane_hypotheses, float* costs, unsigned int* selected_views, const int2 p, const PatchMatchParams params, int checkerboard_iter, int* position, float cost_array[][32], bool* flag) {
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
        return;
...
__device__ void CheckerboardPropagationTexture(const cudaTextureObject_t* images, const cudaTextureObject_t* depths, const Camera* cameras, float4* plane_hypotheses, float* costs, float* pre_costs, curandState* rand_states, unsigned int* selected_views, float4* prior_planes, unsigned int* plane_masks, const int2 p, const PatchMatchParams params, const int iter,float* texture_sampling)
{
    ...
    //int left_near, left_far, right_near, right_far, up_near, up_far, down_near, down_far, left_up, right_up, right_down, left_down;
    //float texture_degree;
    //texture_degree = GetTextureValue(texture_sampling, p, width, height);
    // if (texture_degree < 50) { //original set
    ...
    // }
    // else if (texture_degree >= 50) { // && texture_degree < 100
    ...
    //else if (texture_degree >= 100 && texture_degree < 180) {
    //    int left_near = center - 4;
    //    int left_far = center - 6;
    //    int right_near = center + 4;
    //    int right_far = center + 6;
    //    int up_near = center - 4*width;
    //    int up_far = center - 6 * width;
    //    int down_near = center + 4 * width;
    //    int down_far = center + 6 * width;
    //}
    //else if (texture_degree >= 180 ) {
    //    int left_near = center - 6;
    //    int left_far = center - 8;
    //    int right_near = center + 6;
    //    int right_far = center + 8;
    //    int up_near = center - 6 * width;
    //    int up_far = center - 8 * width;
    //    int down_near = center + 6 * width;
    //    int down_far = center + 8 * width;
    //}
...
#pragma omp parallel for
    ...
    while (itertimes < 3)
   {
       if (symbol_eight_orientations[0] == true) {
           if (BoarderDeter(iter, itertimes, 0, cost_array, params, flag[0],texture_degree)) {
               BoarderLeftUpPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[1] == true) {
           if (BoarderDeter(iter, itertimes, 1, cost_array, params, flag[1],texture_degree)) {
               BoarderUpFarPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[2] == true) {
           if (BoarderDeter(iter, itertimes, 2, cost_array, params, flag[2],texture_degree)) {
               BoarderRightUpPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[3] == true) {
           if (BoarderDeter(iter, itertimes, 3, cost_array, params, flag[3],texture_degree)) {
               BoarderDownFarPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[4] == true) {
           if (BoarderDeter(iter, itertimes, 4, cost_array, params, flag[4],texture_degree)) {
               BoarderRightDownPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[5] == true) {
           if (BoarderDeter(iter, itertimes, 5, cost_array, params, flag[5],texture_degree)) {
               BoarderLeftFarPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[6] == true) {
           if (BoarderDeter(iter, itertimes, 6, cost_array, params, flag[6],texture_degree)) {
               BoarderLeftDownPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
       ...
       if (symbol_eight_orientations[7] == true) {
           if (BoarderDeter(iter, itertimes, 7, cost_array, params, flag[7],texture_degree)) {
               BoarderRightFarPropagation(images, cameras, plane_hypotheses, costs, selected_views, p, params, itertimes, positions_tmp, cost_array, flag);
           }
           else {
...
#if TEXTURE_PROPAGATION
__global__ void BlackPixelUpdateTexture(cudaTextureObjects* texture_objects, cudaTextureObjects* texture_depths, Camera* cameras, float4* plane_hypotheses, float* costs, float* pre_costs, curandState* rand_states, unsigned int* selected_views, float4* prior_planes, unsigned int* plane_masks, const PatchMatchParams params, const int iter,float* texture_sampling)
{
    ...
    CheckerboardPropagationTexture(texture_objects[0].images, texture_depths[0].images, cameras, plane_hypotheses, costs, pre_costs, rand_states, selected_views, prior_planes, plane_masks, p, params, iter,texture_sampling);
}
#else
...
#if TEXTURE_PROPAGATION
__global__ void RedPixelUpdateTexture(cudaTextureObjects* texture_objects, cudaTextureObjects* texture_depths, Camera* cameras, float4* plane_hypotheses, float* costs, float* pre_costs, curandState* rand_states, unsigned int* selected_views, float4* prior_planes, unsigned int* plane_masks, const PatchMatchParams params, const int iter,float* texture_sampling)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    if (threadIdx.x % 2 == 0) {
        p.y = p.y * 2 + 1;
...
void GST::RunPatchMatch()
{
...
#if TEXTURE_BASED_SAMPLING
    RandomInitialization <<<grid_size_randinit, block_size_randinit >>> (texture_objects_cuda, cameras_cuda, plane_hypotheses_cuda, scaled_plane_hypotheses_cuda, costs_cuda, pre_costs_cuda, rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params,texture_obj_sampling_cuda);
#else
    ...
    //     printf("RandomInitialization Error: %s\n", cudaGetErrorString(error1));
    // }

    for (int i = 0; i < max_iterations; ++i) {
#if TEXTURE_PROPAGATION
        BlackPixelUpdateTexture <<<grid_size_checkerboard, block_size_checkerboard >>> (texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, pre_costs_cuda, rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params, i,texture_obj_sampling_cuda);
#else
        ...
        //     printf("Black Error: %s\n", cudaGetErrorString(error2));
        // }
#if TEXTURE_PROPAGATION
        RedPixelUpdateTexture << <grid_size_checkerboard, block_size_checkerboard >> > (texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, pre_costs_cuda, rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params, i,texture_obj_sampling_cuda);
#else
